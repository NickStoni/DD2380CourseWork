...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        EpsilonScheduler = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = EpsilonScheduler.value(current_total_steps)
        p = np.random.random(1)[0]
        if p <= epsilon:
            # It is recommended you use the np.random module
            action = np.random.choice(all_actions)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        discount = self.gamma
        AlphaScheduler = ScheduleLinear(self.annealing_timesteps, 0.3, self.alpha)
        N = 8
        threshold_reward = 6
        q = deque()
        ...
        while episode <= self.episode_max and diff > self.threshold and not similar_reward:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q=Q, state=s_current, all_actions=list_pos, current_total_steps=current_total_steps,
                                        eps_type="linear")
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                lr = AlphaScheduler.value(current_total_steps)
            ...
            if len(q) > N:
                q.popleft()
                reward_diff = max(q) - min(q)
                similar_reward = reward_diff <= threshold_reward
